...
/// Represents a condition, which need to be met in order for the App Prompter dialog to open.
abstract class Condition {
  ...
  bool onEventOccurred(AppPrompterEventType eventType) => false;
}
...
class MinimumDaysCondition extends DebuggableCondition {
  ...
  @override
  bool onEventOccurred(AppPrompterEventType eventType) {
    if (eventType == AppPrompterEventType.laterButtonPressed ||
        eventType == AppPrompterEventType.iOSRequestReview) {
...
class MinimumAppLaunchesCondition extends DebuggableCondition {
  ...
  @override
  bool onEventOccurred(AppPrompterEventType eventType) {
    if (eventType == AppPrompterEventType.initialized) {
      launches++;
      return true;
    }
...
class DoNotOpenAgainCondition extends DebuggableCondition {
  ...
  @override
  bool onEventOccurred(AppPrompterEventType eventType) {
    if (eventType == AppPrompterEventType.actionButtonPressed ||
        eventType == AppPrompterEventType.noButtonPressed) {
      doNotOpenAgain = true;
      return true;
    }
